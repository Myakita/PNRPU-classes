...
switch ($operation) {
	...
	case 'view1':
        ...
        $sql = "CREATE OR REPLACE VIEW $nameview AS
                SELECT ФИО, Телефон, ЗП 
                FROM сотрудники 
                WHERE ЗП > 50000";
        break;
    ...
    case 'view2':
		$nameview = $_POST['nameview'];
        $sql = "CREATE OR REPLACE VIEW $nameview AS
                SELECT ФИО, Телефон, Трудовая_деятельность
                FROM сотрудники 
                WHERE Трудовая_деятельность > 10";
        break;
	...
	case 'insert_proc':
		...
		$sql = "CALL insert_employee('$name', '$phone', $salary, '$address', $years)";
		break;
	...
	case 'view_procs':
		$sql = "SHOW PROCEDURE STATUS WHERE Db = 'mokrushin'";
    break;
