// hash-таблицы.cpp : Этот файл содержит функцию "main". Здесь начинается и заканчивается выполнение программы.
//
...
int main()
{
	...
	cout << "Массив элементов хэш таблицы: ";
	for (int i = 0; i < M; i++)
	{
		hashtable[i] = generateFullName() + "  |  " + bday + "  |  " + generatePassportNum();
	}
	for (int i = 0; i < M; i++)
	{
		cout << hashtable[i]
